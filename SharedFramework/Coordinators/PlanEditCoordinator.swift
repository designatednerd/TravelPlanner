...
public class PlanEditCoordinator {
    ...
    public init(plan: Plan, initialMode: PlanViewMode, intentDonor: IntentDonor?) {
        self.plan = plan
        ...
        let embedded = PlanEditCoordinator.editControllerType(for: plan).loadFromStoryboard()
        embedded.mode = initialMode
